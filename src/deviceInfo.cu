...
struct Ray2 { 
 float3 orig; // ray2 origin
 float3 dir;  // ray2 direction 
 __device__ Ray2(float3 o_, float3 d_) : orig(o_), dir(d_) {} 
};
...
__device__ float intersect_sphere(const Ray2 &r) const { 
          
 // ray2/sphere intersection
 // returns distance t to intersection point, 0 if no hit  
 // ray2 equation: p(x,y,z) = ray2.orig + t*ray2.dir
 // general sphere equation: x^2 + y^2 + z^2 = rad^2 
 // classic quadratic equation of form ax^2 + bx + c = 0 
 // solution x = (-b +- sqrt(b*b - 4ac)) / 2a
 // solve t^2*ray2.dir*ray2.dir + 2*t*(orig-p)*ray2.dir + (orig-p)*(orig-p) - rad*rad = 0 
 // more details in "Realistic Ray2 Tracing" book by P. Shirley or Scratchapixel.com

  float3 op = pos - r.orig;    // distance from ray2.orig to center sphere 
  ...
  return (t = b - disc)>epsilon ? t : ((t = b + disc)>epsilon ? t : 0); // pick closest point in front of ray2 origin
 }
};
...
__device__ inline bool intersect_scene(const Ray2 &r, float &t, int &id){
 ...
 for (int i = int(n); i--;)  // test all scene objects for intersection
  if ((d = spheres[i].intersect_sphere(r)) && d<t){  // if newly computed intersection distance d is smaller than current closest intersection distance
    t = d;  // keep track of distance along ray2 to closest intersection point 
...
// random number generator from https://github.com/gz/rust-ray2tracer
...
__device__ float3 radiance(Ray2 &r, unsigned int *s1, unsigned int *s2){ // returns ray2 color

 float3 accucolor = make_float3(0.0f, 0.0f, 0.0f); // accumulates ray2 colour with each iteration through bounce loop
 float3 mask = make_float3(1.0f, 1.0f, 1.0f); 

 // ray2 bounce loop (no Russian Roulette used) 
...
// test ray2 for intersection with scene
  ...
  // generate new diffuse ray2:
  // origin = hitpoint of previous ray2 in path
  // random direction in hemisphere above hitpoint (see "Realistic Ray2 Tracing", P. Shirley)
  ...
  // compute local orthonormal basis uvw at hitpoint to use for calculation random ray2 direction 
  ...
  // compute random ray2 direction on hemisphere using polar coordinates
  // cosine weighted importance sampling (favours ray2 directions closer to normal direction)
  float3 d = normalize(u*cos(r1)*r2s + v*sin(r1)*r2s + w*sqrtf(1 - r2));

  // new ray2 origin is intersection point of previous ray2 with scene
  r.orig = x + nl*0.05f; // offset ray2 origin slightly to prevent self intersection
  r.dir = d;

  mask = mask * obj.col;    // multiply with colour of object       
...
// generate ray2 directed at lower left corner of the screen
// compute directions for all other ray2s by adding cx and cy increments in x and y direction
 Ray2 cam(make_float3(50, 52, 295.6), normalize(make_float3(0, -0.042612, -1))); // first hardcoded camera ray2(origin, direction) 
 float3 cx = make_float3(width * .5135 / height, 0.0f, 0.0f); // ray2 direction offset in x direction
 float3 cy = normalize(cross(cx, cam.dir)) * .5135; // ray2 direction offset in y direction (.5135 is field of view angle)
 ...
 for (int s = 0; s < samps; s++){  // samples per pixel
  ...
  // compute primary ray2 direction
  float3 d = cam.dir + cx*((.25 + x) / width - .5) + cy*((.25 + y) / height - .5);
  
  // create primary ray2, add incoming radiance to pixelcolor
  Ray2 r2(cam.orig + d * 40, normalize(d));
  r = r + radiance(r2, &s1, &s2)*(1. / samps); 
 }       // Camera ray2s are pushed ^^^^^ forward to start in interior 
